...
2. Öffnen Sie `scripts/odd-even-backup.ps1` in einem Texteditor (z. B. Notepad) und passen Sie folgende Werte an:
   ...
   - `LogDirectory` (optional): Speicherort für die Robocopy-Protokolle.
...
3. Die Oberfläche zeigt Ihnen Quelle und aktuelle Ziel-Festplatte an. Starten Sie die Sicherung über **Backup starten**. Während des Kopiervorgangs bleibt das Fenster geöffnet und zeigt den Logpfad sowie die letzten Zeilen der Robocopy-Protokolldatei an.
...
- Die Robocopy-Protokolle finden Sie im Ordner, den Sie über `LogDirectory` festgelegt haben.
